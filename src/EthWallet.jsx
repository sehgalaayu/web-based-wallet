...
const EthWallet = forwardRef(function EthWallet(
  {
    mnemonic,
    copyToClipboard,
    onWalletAdded,
  ...
  const addWallet = async (mnemonicOverride) => {
    ...
    try {
      ...
      setWallets((prev) => [
        ...prev,
        { address: wallet.address, priv: privateKey },
      ]);
      setRevealedPrivate((prev) => [...prev, false]);
      setCurrentIndex((prev) => prev + 1);
      onWalletAdded?.({ network: "Ethereum", address: wallet.address });
      return wallet.address;
    } catch (error) {
      console.error("Failed to generate ETH wallet:", error);
      showToast("Failed to add ETH wallet", "error");
      return null;
    }
  };
  ...
  useImperativeHandle(ref, () => ({
    addWallet,
    getFirstAddress: () => wallets[0]?.address,
    toggleAllPrivate,
    resetWallets,
  }));
    ...
    <section>
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-[12px] text-[var(--text-2)] opacity-80">
            Manage ETH accounts and secure private keys.
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            className="rounded-lg bg-gradient-to-r from-emerald-500 to-emerald-600 px-4 py-2 text-xs font-medium text-white transition-all duration-300 hover:shadow-lg hover:shadow-emerald-500/30 hover:scale-105 active:scale-95"
            onClick={() => addWallet()}
          >
            + Add
          </button>
      ...
      {(!mnemonic || !canAddWallet) && (
        <p className="mb-2 text-[11px] text-[var(--text-3)]">
          Connect wallet first, then add ETH accounts.
        </p>
      )}
      ...
      {wallets.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          {wallets.map((wallet, index) => (
            <article
              key={wallet.address}
              className="wallet-lift rounded-[12px] border border-white/10 bg-gradient-to-br from-[rgba(18,18,18,0.96)] via-[rgba(14,14,14,0.9)] to-[rgba(11,11,11,0.9)] p-5 shadow-[0_12px_32px_rgba(0,0,0,0.45)] backdrop-blur-sm transition-all duration-300 hover:-translate-y-[1px] hover:border-emerald-500/25 hover:shadow-[0_18px_42px_rgba(0,0,0,0.55)]"
            >
              <div className="mb-4 flex items-start justify-between">
                ...
                <div className="text-right">
                  <p className="text-[13px] font-semibold text-emerald-400">
                    ${(0.42 + index * 0.13).toFixed(2)} ETH
                  </p>
                  <p className="text-[10px] text-[var(--text-3)]">
                    24h +{(1.2 + index * 0.4).toFixed(1)}%
                  </p>
                </div>
              </div>
              ...
              <div className="mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    onClick={() => copyFullAddressAction(wallet.address, index)}
